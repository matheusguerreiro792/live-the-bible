...
<template>
  <main class="flex flex-col flex-1 bg-neutral-900 text-neutral-50 px-4 gap-6 p-6 items-center w-full max-w-6xl mx-auto">
    <h1 class="text-4xl font-bold text-yellow-500 w-full mb-6">Administração</h1>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8 w-full">
      ...
      <section class="bg-neutral-800 flex flex-col gap-4 p-6 rounded-xl shadow-xl border border-neutral-700">
        <div class="flex justify-between items-center mb-4">
          ...
          <button
            class="bg-green-600 py-2 px-4 rounded-lg font-medium hover:bg-green-700 transition-colors shadow-lg"
        ...
        <div class="flex flex-col gap-3 max-h-[600px] overflow-y-auto pr-2 custom-scrollbar">
          ...
          >
            <div class="flex justify-between items-start">
              ...
              <div class="flex gap-2">
                <button
                  @click="openStudyForm(item)"
                  class="bg-blue-600/20 text-blue-400 p-2 rounded hover:bg-blue-600 hover:text-white transition-all text-xs"
                >
                  Editar
                </button>
                <button
                  @click="handleDeleteStudy(item.id)"
                  class="bg-red-600/20 text-red-400 p-2 rounded hover:bg-red-600 hover:text-white transition-all text-xs"
                >
                  Excluir
                </button>
    ...
    <!-- Modals -->
    <div v-if="showDevotionalForm" class="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <DevotionalForm
        :devotional="editingDevotional"
        @close="closeDevotionalForm"
        @saved="fetchData"
      />
    </div>

    <div v-if="showStudyForm" class="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <StudiesForm
        :studie="editingStudy"
        @close="closeStudyForm"
        @saved="fetchData"
      />
    </div>
  </main>
